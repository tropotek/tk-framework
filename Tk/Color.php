...
class Color
{
    ...
    {
        $regs = null;
        if (preg_match('/^(\#)?([A-F0-9]{3})$/i', $str, $regs)) {   // is 3 char hex
            $str = $regs[2][0] . $regs[2][0] . $regs[2][1] . $regs[2][1] . $regs[2][2] . $regs[2][2];
        }
        if (preg_match('/^(\#)?([A-F0-9]{6})$/i', $str, $regs)) {   // is 6 char string
            $this->hex = $regs[2];
            $this->hex = strtoupper($this->hex);
            return $this;
        }
        if (preg_match('/^([a-z]{3,32})$/i', $str) && array_key_exists($str, self::$colorChart)) {  // is name in the color chart
